...
class IntegratedReactor:
    ...
    def _execute_gsa(self, params, param_keys, rbc_model_key, N_morris=100, N_sobol=512):
        ...
        with Progress(SpinnerColumn(), "[progress.description]{task.description}", BarColumn(), "{task.completed}/{task.total}", console=console) as progress:
            task_m = progress.add_task(
                "[cyan]Running Morris Analysis...", total=len(param_values_morris))
            # Pass only relevant parameters to the wrapper
            Y_morris = np.array([self._gsa_model_wrapper(
                p, param_keys, self.steady_df, rbc_model_key) for p in param_values_morris])
            progress.update(task_m, completed=len(param_values_morris))

            task_s = progress.add_task(
                "[green]Running Sobol Analysis...", total=len(param_values_sobol))
            Y_sobol = np.array([self._gsa_model_wrapper(
                p, param_keys, self.steady_df, rbc_model_key) for p in param_values_sobol])
            progress.update(task_s, completed=len(param_values_sobol))
        ...
        Mi = morris_analyzer.analyze(
            problem, param_values_morris, Y_morris, conf_level=0.95).to_df()
        Si = sobol.analyze(problem, Y_sobol, calc_second_order=True)

        Mi.to_csv(f"gsa_morris_results_{rbc_model_key}.csv")
        Si_df = Si.to_df()
        Si_df[0].to_csv(f"gsa_sobol_total_results_{rbc_model_key}.csv")
        Si_df[1].to_csv(f"gsa_sobol_first_results_{rbc_model_key}.csv")
        TermFX.print_success("GSA results saved to CSV files.")
        self.visualize_gsa_results(Mi, Si, problem)
    ...
    def _execute_mc(self, params, param_keys, rbc_model_key, N=1000):
        ...
        outputs = []
        with Progress(SpinnerColumn(), "[progress.description]{task.description}", BarColumn(), "{task.completed}/{task.total}", console=console) as progress:
            task = progress.add_task("[cyan]Running simulations...", total=N)
            for i in range(N):
                param_vector = samples_df.iloc[i].values
                outputs.append(self._gsa_model_wrapper(
                    param_vector, param_keys, self.steady_df, rbc_model_key))
        ...
        results_df.to_csv(
            f"mc_sensitivity_results_{rbc_model_key}.csv", index=False)
        TermFX.print_success(
            "Monte Carlo analysis complete and results saved.")
...
# ==============================================================================
# 6. Main Execution Block & Interactive Loop
# ==============================================================================


def run_interactive_mode():
    TermFX.animated_startup_banner()
    CONFIG = {
        "data_file": "Model_Data.csv", "reactor_constants": {"V_RBC": 18, "A_RBC": 240},
        "optimization_bounds": {
            "UASB": [(1.4, 75.9), (4.9, 25.0), (1.9, 8.0)], "Filter": [(0.01, 4.0), (0.01, 2.5)],
            "RBC": [(5, 30), (100, 500), (20, 60), (1, 15), (300, 450), (1, 10), (0.01, 1), (0.01, 2.5)],
            "RBC_pH": [(5, 50), (50, 100), (20, 30), (1, 5), (1, 5), (1, 10), (0.01, 2.5), (0.01, 2.5), (3.0, 5), (9, 11.5)]}}

    reactor_model = IntegratedReactor(
        CONFIG["data_file"], CONFIG["reactor_constants"])
    if not reactor_model.load_and_prepare_data():
        return

    while True:
        console.print("\n")
        menu = Table(title="[bold]MAIN MENU[/bold]", style="purple",
                     title_style="bold magenta", show_header=False)
        menu.add_column(justify="right", style="cyan")
        menu.add_column()
        menu.add_row("[1]", "Data Exploration & Configuration")
        menu.add_row("[2]", "Parameter Estimation")
        menu.add_row("[3]", "Model Performance & Visualization" +
                     ("" if reactor_model.params_estimated else " [dim](Disabled)[/dim]"))
        menu.add_row("[4]", "Sensitivity Analysis" +
                     ("" if reactor_model.params_estimated else " [dim](Disabled)[/dim]"))
        menu.add_row("[5]", "File & Data Management")
        menu.add_row("[6]", "Exit")
        console.print(Panel(menu, border_style="purple", expand=False))
        choice = console.input(
            "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

        if choice == '1':
            while True:
                console.print()
                exp_menu = Table(show_header=False, box=None,
                                 title="[bold]Data Exploration & Configuration Menu[/bold]")
                exp_menu.add_column(style="magenta", justify="right")
                exp_menu.add_column(style="white")
                exp_menu.add_row("[1]", "Show Data Summary Statistics")
                exp_menu.add_row("[2]", "Show Correlation Heatmap")
                exp_menu.add_row("[3]", "View Current Configuration")
                exp_menu.add_row("[4]", "Return to Main Menu")
                console.print(
                    Panel(exp_menu, border_style="cyan", expand=False))
                exp_choice = console.input(
                    "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

                if exp_choice == '1':
                    reactor_model.show_data_summary()
                elif exp_choice == '2':
                    reactor_model.show_correlation_heatmap()
                elif exp_choice == '3':
                    reactor_model.view_configuration(CONFIG)
                elif exp_choice == '4':
                    break
                else:
                    TermFX.print_error("Invalid selection.")
            ...
            while True:
                console.print()
                est_menu = Table(show_header=False, box=None,
                                 title="[bold]Parameter Estimation Menu[/bold]")
                est_menu.add_column(style="magenta", justify="right")
                est_menu.add_column(style="white")
                est_menu.add_row("[1]", "Run New Estimation")
                est_menu.add_row("[2]", "Load Parameters from File")
                est_menu.add_row("[3]", "Save Current Parameters" + (
                    "" if reactor_model.params_estimated else " [dim](Disabled)[/dim]"))
                est_menu.add_row("[4]", "Display Current Parameters" + (
                    "" if reactor_model.params_estimated else " [dim](Disabled)[/dim]"))
                est_menu.add_row("[5]", "Return to Main Menu")
                console.print(
                    Panel(est_menu, border_style="cyan", expand=False))
                est_choice = console.input(
                    "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

                if est_choice == '1':
                    reactor_model.run_parameter_estimation(
                        CONFIG["optimization_bounds"])
                elif est_choice == '2':
                    reactor_model.load_parameters()
                elif est_choice == '3':
                    reactor_model.save_parameters()
                elif est_choice == '4':
                    reactor_model.display_estimated_parameters()
                elif est_choice == '5':
                    break
                else:
                    TermFX.print_error("Invalid selection.")
            ...
            if reactor_model.params_estimated:
                if not reactor_model.predictions_run:
                    TermFX.print_subheader(
                        "Predictions not found, running simulation first...")
                    reactor_model.run_full_predictions()

                while True:
                    console.print()
                    vis_menu = Table(
                        show_header=False, box=None, title="[bold]Performance & Visualization Menu[/bold]")
                    vis_menu.add_column(style="magenta", justify="right")
                    vis_menu.add_column(style="white")
                    vis_menu.add_row(
                        "[1]", "Show Performance Metrics & Equations Tables")
                    vis_menu.add_row("[2]", "Show Time-Series Plot")
                    vis_menu.add_row("[3]", "Show Parity Plots")
                    vis_menu.add_row("[4]", "Return to Main Menu")
                    console.print(
                        Panel(vis_menu, border_style="cyan", expand=False))
                    vis_choice = console.input(
                        "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

                    if vis_choice == '1':
                        reactor_model.display_performance_and_equations()
                    elif vis_choice == '2':
                        reactor_model.visualize_time_series()
                    elif vis_choice == '3':
                        reactor_model.visualize_parity_plots()
                    elif vis_choice == '4':
                        break
                    else:
                        TermFX.print_error("Invalid selection.")
                ...
                TermFX.print_warning(
                    "Please run or load parameter estimation [2] first.")

        elif choice == '4':
            if reactor_model.params_estimated:
                while True:
                    console.print()
                    sa_menu = Table(show_header=False, box=None,
                                    title="[bold]Sensitivity Analysis Menu[/bold]")
                    sa_menu.add_column(style="magenta", justify="right")
                    sa_menu.add_column(style="white")
                    sa_menu.add_row("[1]", "Run Global Analysis (GSA)")
                    sa_menu.add_row("[2]", "Run Monte Carlo Analysis (MC)")
                    sa_menu.add_row("[3]", "Process GSA Results")
                    sa_menu.add_row("[4]", "Process MC Results")
                    sa_menu.add_row("[5]", "Return to Main Menu")
                    console.print(
                        Panel(sa_menu, border_style="cyan", expand=False))
                    sa_choice = console.input(
                        "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

                    if sa_choice == '1':
                        reactor_model.run_sensitivity_analysis('gsa')
                    elif sa_choice == '2':
                        reactor_model.run_sensitivity_analysis('mc')
                    elif sa_choice in ['3', '4']:
                        analysis_type = "GSA" if sa_choice == '3' else "MC"
                        console.print(
                            f"\n[cyan]Select Model for {analysis_type} Results:[/]\n [1] Original\n [2] pH-Modified")
                        model_choice = console.input(
                            "[bold green]>[/bold green] [cyan]Enter model choice: [/cyan]")
                        model_key = 'ph' if model_choice == '2' else 'orig'

                        console.print(
                            f"\n[cyan]Select Output Format:[/]\n [1] Table\n [2] Chart")
                        output_choice = console.input(
                            "[bold green]>[/bold green] [cyan]Enter format choice: [/cyan]")
                        output_key = 'chart' if output_choice == '2' else 'table'

                        if analysis_type == "GSA":
                            reactor_model.load_and_display_gsa_results(
                                model_key, output_key)
                        else:
                            reactor_model.load_and_display_mc_results(
                                model_key, output_key)
                    elif sa_choice == '5':
                        break
                    else:
            ...
            while True:
                console.print()
                del_menu = Table(show_header=False, box=None,
                                 title="[bold]File & Data Management Menu[/bold]")
                del_menu.add_column(style="magenta", justify="right")
                del_menu.add_column(style="white")
                del_menu.add_row("[1]", "Delete Saved Parameters (`.json`)")
                del_menu.add_row("[2]", "Delete Saved GSA Results (`.csv`)")
                del_menu.add_row("[3]", "Delete Saved MC Results (`.csv`)")
                del_menu.add_row("[4]", "Return to Main Menu")
                console.print(
                    Panel(del_menu, border_style="cyan", expand=False))
                del_choice = console.input(
                    "[bold green]>[/bold green] [cyan]Select an option: [/cyan]")

                if del_choice == '1':
                    reactor_model.delete_files(
                        "calibrated_params.json", "Parameter")
                elif del_choice == '2':
                    reactor_model.delete_files("gsa_*.csv", "GSA Result")
                elif del_choice == '3':
                    reactor_model.delete_files("mc_*.csv", "MC Result")
                elif del_choice == '4':
                    break
                else:
                    TermFX.print_error("Invalid selection.")
            ...
            console.print(
                Panel("[bold magenta]Exiting Simulation Core. Goodbye![/]"))
            break
        else:
            TermFX.print_error("Invalid option. Please try again.")
        time.sleep(1)
